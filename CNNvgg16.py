...
import os
import random
from augmentation import Cutout, CIFAR10Policy, evaluate_accuracy
...
class CNNsteprelu(nn.Module):
    def __init__(self,theta):
        super(CNNsteprelu, self).__init__()
        self.theta=theta
        hooks = []
        cnn = nn.Sequential(
            nn.Conv2d(3, 64, 3, 1, 1), nn.BatchNorm2d(64), VarientRelu(threshold1=theta), nn.Conv2d(64, 64, 3, 1, 1), nn.BatchNorm2d(64), VarientRelu(threshold1=0.05),
            nn.MaxPool2d(2, 2),
            nn.Conv2d(64, 128, 3, 1, 1), nn.BatchNorm2d(128), VarientRelu(threshold1=theta), nn.Conv2d(128, 128, 3, 1, 1), nn.BatchNorm2d(128), VarientRelu(threshold1=0.05),
            nn.MaxPool2d(2, 2),
            nn.Conv2d(128, 256, 3, 1, 1), nn.BatchNorm2d(256), VarientRelu(threshold1=theta), nn.Conv2d(256, 256, 3, 1, 1), nn.BatchNorm2d(256), VarientRelu(threshold1=0.05), nn.Conv2d(256, 256, 3, 1, 1), nn.BatchNorm2d(256), VarientRelu(threshold1=0.05),
            nn.MaxPool2d(2, 2),
            nn.Conv2d(256, 512, 3, 1, 1), nn.BatchNorm2d(512), VarientRelu(threshold1=theta), nn.Conv2d(512, 512, 3, 1, 1), nn.BatchNorm2d(512), VarientRelu(threshold1=0.05), nn.Conv2d(512, 512, 3, 1, 1), nn.BatchNorm2d(512), VarientRelu(threshold1=0.05),
            nn.MaxPool2d(2, 2),
            nn.Conv2d(512, 512, 3, 1, 1), nn.BatchNorm2d(512), VarientRelu(threshold1=theta), nn.Conv2d(512, 512, 3, 1, 1), nn.BatchNorm2d(512), VarientRelu(threshold1=0.05), nn.Conv2d(512, 512, 3, 1, 1), nn.BatchNorm2d(512), VarientRelu(threshold1=0.05),
            nn.MaxPool2d(2, 2))
...
if __name__ == '__main__':
    seed_all(42)
    batch_size = 128
    savepath='./model/CNNvgg16Cifar100.pt'
    ...
    train_iter = torch.utils.data.DataLoader(cifar100_train, batch_size=batch_size, shuffle=True, num_workers=0, pin_memory=True)
    test_iter = torch.utils.data.DataLoader(cifar100_test, batch_size=batch_size, shuffle=False, num_workers=0, pin_memory=True)
    ...
    train(net, train_iter=train_iter, test_iter=test_iter, optimizer=optimizer, scheduler=scheduler, device=device, num_epochs=num_epochs, losstype='crossentropy')
